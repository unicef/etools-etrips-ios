...
public final class TripEntity: ManagedObject {
    ...
    @NSManaged private(set) public var startDate: Date?
    @NSManaged private(set) public var endDate: Date?
    ...
    public static func findAndUpdateOrCreate(in context: NSManagedObjectContext,
        ...
        let tripEntity = findAndUpdateOrCreate(in: context, with: predicate) {
            ...
            if let purposeOfTravel = object.purposeOfTravel {
                $0.purposeOfTravel = purposeOfTravel
            }
    ...
    public func deleteLocalReport(in context: NSManagedObjectContext) {
        if let localReport = localReport {
            context.delete(localReport)
        }

        if let localFiles = localFiles {
            for localFile in localFiles {
                context.delete(localFile)
            }
        }
    }

    var fromDateToDateString: String {
        guard let startDateString = startDate?.mediumDateStyleString(),
            let endDateString = endDate?.mediumDateStyleString() else {
            return ""
        }
