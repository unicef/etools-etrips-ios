...
class CoreDataStack {
    ...
    public func cleanData() {
        let removableEntityRequest = NSFetchRequest<NSFetchRequestResult>(entityName: "RemovableEntity")
        let deleteTripsRequest = NSBatchDeleteRequest(fetchRequest: removableEntityRequest)
        try! managedObjectContext.execute(deleteTripsRequest)
        
        // Delete users separate because they don't inherite from `RemovableEntity`.
        let userEntityRequest = NSFetchRequest<NSFetchRequestResult>(entityName: UserEntity.entityName)
        let deleteUsersRequest = NSBatchDeleteRequest(fetchRequest: userEntityRequest)
        try! managedObjectContext.execute(deleteUsersRequest)
